...
Deno.test('old examples from docs', async (t) => {
	const uuids = [
	...
	await t.step('myFancyRegex', () => {
		...
		assertEquals(myFancyRegex, expected)
	})

	await t.step('myInterpolatedRegex', () => {
		const expected = /^abc\.hello, world!🌎\w\d\b\0\\...\r\n\t\x20$/iu

		assertEquals(myInterpolatedRegex, expected)
	})

	await t.step('myRegexWithOptions', () => {
		const expected = /^💩+$/gu

		assertEquals(myRegexWithOptions, expected)
	})
	...
	await t.step('rawInterpolation', () => {
		...
		const rawInterpolation = regex()`
			${new RegexFragment('.')}
		`
		assertEquals(rawInterpolation, expected)
	})
	...
	await t.step('withArray old', () => {
		...
		`
		assertEquals(withArray, expected)
	})
